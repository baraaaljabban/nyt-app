...
mixin ErrorHandler {
  ...
  Future<CustomException> getNonSuccessHttpResponseException(Response response) async {
    //TODO: get it from localization when you have time
    const String SERVICE_UNAVAILABLE = 'service unavailable';
    const String FORBIDDEN = 'forbidden';
    const String MFA = 'mfa';
    ...
    final failedResponse = FailedResponse.fromMap(json, response: response);
    CustomException exceptionToThrow = UnhandledServerErrorException(statusCode: failedResponse.status, body: failedResponse.message);
    if (failedResponse.data.containsKey(MFA) && failedResponse.data[MFA]) {
      exceptionToThrow = LoginMfaRequiredException(
      ...
      throw ForbiddenException(body: failedResponse.message, statusCode: failedResponse.status);
    } else if (failedResponse.status == 403 && response.reasonPhrase?.toLowerCase() == FORBIDDEN) {
      throw ForbiddenException(body: failedResponse.message, statusCode: failedResponse.status);
    } else if (failedResponse.status == 503 && response.reasonPhrase?.toLowerCase() == SERVICE_UNAVAILABLE) {
      throw ServerUnavailableException(statusCode: failedResponse.status);
  ...
  Failure mapCommonExceptionToFailure(Exception exception) {
    ...
    if (exception is LoginMfaRequiredException) {
      debugPrint("LoginMfaRequiredException ${exception.message}");
      return MfaRequiredFailure(message: exception.message, title: exception.title);
    }
    ...
    if (exception is ServerUnavailableException) {
      debugPrint("ServerUnavailableException ${exception.statusCode}");
      return ServerUnavailableFailure(statusCode: exception.statusCode, message: exception.body);
    }
